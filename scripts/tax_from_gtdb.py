...
---
Made several modifications to the original script to avoid some errors. This script only works
with building kraken2 index.
...
def get_arguments():
    ...
    input_args.add_argument('--tree', type=str,
                            help='If provided, this script will only use assemblies that are in '
                                 'the tree (for building with the dereplicated genomes)') # this option is dyfunc in this script
...
def get_embl_code(taxon):
    if taxon.startswith('s__'):
        ...
        try:
            s_letter = taxon[3:].split('_')[1][0].upper()
        except IndexError:
            s_letter = ''
...
def get_assembly_filename(accession, all_assemblies):
    ...
    if len(assembly_filenames) == 0:
        ...
        # comment out so duplicate accid can exist, and the latest version is used.
